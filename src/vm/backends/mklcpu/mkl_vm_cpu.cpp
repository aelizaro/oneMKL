...
cl::sycl::event add(cl::sycl::queue &q, std::int64_t n, cl::sycl::buffer<std::complex<double>> &a,
    ...
    auto event = q.submit([&](cl::sycl::handler &cgh) {
        ...
        host_task<class mkl_kernel_zadd>(cgh, [=]() {
            MKL_Complex16 *a_ptr =
                reinterpret_cast<MKL_Complex16 *>(accessor_a.get_pointer().get());
            MKL_Complex16 *b_ptr =
                reinterpret_cast<MKL_Complex16 *>(accessor_b.get_pointer().get());
            MKL_Complex16 *y_ptr =
                reinterpret_cast<MKL_Complex16 *>(accessor_y.get_pointer().get());
...
cl::sycl::event add(cl::sycl::queue &q, std::int64_t n, cl::sycl::buffer<float> &a,
    ...
    auto event = q.submit([&](cl::sycl::handler &cgh) {
        ...
        host_task<class mkl_kernel_sadd>(cgh, [=]() {
            ::vmsAdd(n, accessor_a.get_pointer(), accessor_b.get_pointer(),
                     accessor_y.get_pointer(), classic_mode);
        });
    });
    return event;
}
cl::sycl::event add(cl::sycl::queue &q, std::int64_t n, cl::sycl::buffer<double> &a,
    ...
    auto event = q.submit([&](cl::sycl::handler &cgh) {
        ...
        host_task<class mkl_kernel_dadd>(cgh, [=]() {
            ::vmdAdd(n, accessor_a.get_pointer(), accessor_b.get_pointer(),
                     accessor_y.get_pointer(), classic_mode);
        });
    });
    return event;
...
cl::sycl::event add(cl::sycl::queue &q, std::int64_t n, std::complex<float> *a,
    ...
    auto event = q.submit([&](cl::sycl::handler &cgh) {
        host_task<class mkl_kernel_cadd_usm>(cgh, [=]() {
            ::vmcAdd(n, reinterpret_cast<MKL_Complex8 *>(a), reinterpret_cast<MKL_Complex8 *>(b),
                     reinterpret_cast<MKL_Complex8 *>(y), classic_mode);
        });
    });
    return event;
}
cl::sycl::event add(cl::sycl::queue &q, std::int64_t n, std::complex<double> *a,
    ...
    auto event = q.submit([&](cl::sycl::handler &cgh) {
        host_task<class mkl_kernel_cadd_usm>(cgh, [=]() {
            ::vmzAdd(n, reinterpret_cast<MKL_Complex16 *>(a), reinterpret_cast<MKL_Complex16 *>(b),
                     reinterpret_cast<MKL_Complex16 *>(y), classic_mode);
        });
    });
    return event;
}
cl::sycl::event add(cl::sycl::queue &q, std::int64_t n, float *a, float *b, float *y,
    ...
    auto classic_mode = get_classic_mode(given_mode);
    auto event = q.submit([&](cl::sycl::handler &cgh) {
        host_task<class mkl_kernel_sadd_usm>(cgh, [=]() { ::vmsAdd(n, a, b, y, classic_mode); });
    });
    return event;
}
...
cl::sycl::event add(cl::sycl::queue &q, std::int64_t n, double *a, double *b, double *y,
    ...
    auto classic_mode = get_classic_mode(given_mode);
    auto event = q.submit([&](cl::sycl::handler &cgh) {
        host_task<class mkl_kernel_dadd_usm>(cgh, [=]() { ::vmdAdd(n, a, b, y, classic_mode); });
    });
    return event;
}
