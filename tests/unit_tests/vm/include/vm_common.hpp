...
#ifndef _VM_TEST_COMMON_HPP__
#define _VM_TEST_COMMON_HPP__
...
//! ===========================================================================
template <typename T>
static std::string get_typename(T arg) {}
template <>
std::string get_typename(float arg) {
    std::string name = "Single";
    return name;
}
template <>
std::string get_typename(double arg) {
    std::string name = "Double";
    return name;
}
template <>
std::string get_typename(std::complex<float> arg) {
    std::string name = "Complex Single";
    return name;
}
template <>
std::string get_typename(std::complex<double> arg) {
    std::string name = "Complex Double";
    return name;
}

//! ===========================================================================
//! @brief Get maximum ulp value.
//!
//! @param[in] x     First ulp argument
//! @param[in] y     Second ulp argument
//!
//! @return  Absolute maximum ulp value
//! ===========================================================================
template <typename T>
T static get_max_ulp(T x, T y) {
    return std::max(std::abs(x), std::abs(y));
}
template <>
std::complex<float> get_max_ulp(std::complex<float> x, std::complex<float> y) {
    float remax = std::max(std::abs(x.real()), std::abs(y.real()));
    float immax = std::max(std::abs(x.imag()), std::abs(y.imag()));
    std::complex<float> res(remax, immax);
    return res;
}
template <>
std::complex<double> get_max_ulp(std::complex<double> x, std::complex<double> y) {
    double remax = std::max(std::abs(x.real()), std::abs(y.real()));
    double immax = std::max(std::abs(x.imag()), std::abs(y.imag()));
    std::complex<float> res(remax, immax);
    return res;
}
...
//! ===========================================================================
template <typename T>
static T compute_ulp(T res, T ref) {}
// float specialization
template <>
float compute_ulp(float res, float ref) {
    float retval;
...
template <>
double compute_ulp(double res, double ref) {
    double retval;
    const int res_class = std::fpclassify(res);
    const int ref_class = std::fpclassify(ref);
...
template <>
std::complex<float> compute_ulp(std::complex<float> res, std::complex<float> ref) {
    ...
    if (std::isfinite(res.real()) && std::isfinite(ref.real()) && std::isfinite(res.imag()) &&
        std::isfinite(ref.imag()))
...
template <>
std::complex<double> compute_ulp(std::complex<double> res, std::complex<double> ref) {
...
//! ===========================================================================
template <typename T>
static bool check_ulp(T ulp, bool exact, oneapi::mkl::vm::mode acc,
                      ulp_table_type const &ulp_table) {
    return true;
}
// float specialization
template <>
bool check_ulp(float ulp, bool exact, oneapi::mkl::vm::mode acc, ulp_table_type const &ulp_table) {
    float max_ulp = (acc == accuracy_mode[ACCURACY_EP])
                        ? ulp_table.at(MAX_EP_ULP_S)
...
// double specialization
template <>
bool check_ulp(double ulp, bool exact, oneapi::mkl::vm::mode acc, ulp_table_type const &ulp_table) {
    double max_ulp = (acc == accuracy_mode[ACCURACY_EP])
                         ? ulp_table.at(MAX_EP_ULP_D)
...
// complex float specialization
template <>
bool check_ulp(std::complex<float> ulp, bool exact, oneapi::mkl::vm::mode acc,
               ulp_table_type const &ulp_table) {
    float max_ulp = (acc == accuracy_mode[ACCURACY_EP])
                        ? ulp_table.at(MAX_EP_ULP_C)
...
// complex double specialization
template <>
bool check_ulp(std::complex<double> ulp, bool exact, oneapi::mkl::vm::mode acc,
               ulp_table_type const &ulp_table) {
    double max_ulp = (acc == accuracy_mode[ACCURACY_EP])
                         ? ulp_table.at(MAX_EP_ULP_Z)
...
int check_result(char const *device, char const *funcname, char const *indexing, int idx,
                 int argtype, int acc, A &arg1, A &arg2, R &res1, R &res2, R &ref1, R &ref2,
                 ulp_table_type const &ulp_table, bool detailed_output, bool only_errors) {
    std::stringstream strout;
    R ulp = compute_ulp<R>(res1, ref1);
    if (argtype == ARG1_RES2) {
        R ulp2 = compute_ulp<R>(res2, ref2);
        ulp = get_max_ulp(ulp, ulp2);
    }

    bool good_ulp = check_ulp(ulp, false, accuracy_mode[acc], ulp_table);

    if ((only_errors == false) || (good_ulp == false)) {
        if (detailed_output == true) {
            if (good_ulp == false) {
                strout << "ERROR:";
            }
            else {
                strout << "CHECKED VALUE:";
            }

            strout << " ,domain = ,VM"
                   << " ,test = ,Accuracy"
                   << " ,device = ," << device << " ,function = ," << funcname << " ,precision =  ,"
                   << get_typename<A>(arg1) << " ,accuracy = , " << accuracy_name[acc]
                   << " ,indexing = , " << indexing;

            strout << " ,arg1[" << idx << "] = ," << arg1;
            if (argtype == ARG2_RES1) {
                strout << " ,arg2 = ," << arg2;
            }
            strout << " ,computed res = ," << res1 << " ,expected res = ," << ref1;
            if (argtype == ARG1_RES2) {
                strout << " ,computed res2 = ," << res2 << " ,expected res2 = ," << ref2;
            }
            strout << ",ulp = " << ulp << std::endl;
        } // if(detailed_output == true)
        else {
            strout << "\t\t" << funcname << "<" << accuracy_name[acc] << indexing << ">[" << idx
                   << "]";
            if (argtype == ARG2_RES1) {
                strout << "({" << arg1 << "," << arg2 << "}) = ";
            }
...
struct data_7_t {
    ...
    void get_values(double &v1, double &v2, double &v3, double &v4, double &v5, double &v6,
                    double &v7) {
        v1 = data_f64[i].v1.f;
        v2 = data_f64[i].v2.f;
        v3 = data_f64[i].v3.f;
...
template <typename T>
T *own_malloc_device(cl::sycl::queue &q, size_t s) {
...
template <typename T>
T *own_malloc(cl::sycl::queue &q, size_t s) {
...
template <typename T>
void own_free_device(cl::sycl::queue &q, T *obj) {
    cl::sycl::free(obj, q);
}

template <typename T>
void own_free(cl::sycl::queue &q, T *obj) {
    if (q.get_device().is_gpu()) {
        cl::sycl::free(obj, q);
    }
    else {
...
template <typename T>
T *safe_copy_from_device(T *dev_begin, T *dev_end, T *dst, cl::sycl::queue &q) {
    if (dev_end < dev_begin) {
        throw std::runtime_error("Safe copy received end < begin");
    }
    if (nullptr == dst) {
        throw std::runtime_error("Safe copy received dst == nullptr");
    }
    ...
    try {
        ...
        if (nullptr == host_ptr) {
            throw std::runtime_error("USM memory allocation error");
        }
        ...
        std::cout << "SYCL exception during memcpy from device\n"
                  << e.what() << std::endl
...
template <typename T>
T *safe_copy_to_device(T *begin, T *end, T *dev_dst, cl::sycl::queue &q) {
    if (end < begin) {
        throw std::runtime_error("Safe copy received end < begin");
    }
    if (nullptr == dev_dst) {
        throw std::runtime_error("Safe copy received dev_dst == nullptr");
    }
