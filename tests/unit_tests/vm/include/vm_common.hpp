...
//! ===========================================================================
template <typename T> T static get_max_ulp(T x, T y){ return std::max(std::abs(x), std::abs(y)); }
template <> std::complex <float> get_max_ulp(std::complex <float> x, std::complex <float> y)
{ 
...
template <> std::complex <double> get_max_ulp(std::complex <double> x, std::complex <double> y)
{ 
...
template <typename A, typename R> int check_result (char const * device, char const * funcname, char const * indexing, 
                                        int idx, int argtype, int acc, 
                                        A & arg1, A & arg2, R & res1, R & res2, R & ref1, R & ref2, 
                                        ulp_table_type const & ulp_table, 
                                        bool detailed_output, bool only_errors)
{
    ...
    if ((only_errors == false) || (good_ulp == false))
    {
        if(detailed_output == true)
        {
            ...
            strout  << " ,domain = ,VM" << " ,test = ,Accuracy"  << " ,device = ," << device  << 
            " ,function = ," << funcname  << " ,precision =  ," << get_typename<A>(arg1)  << 
            " ,accuracy = , " << accuracy_name[acc] << " ,indexing = , " << indexing;
    ...
    if (only_errors == true){ std::cerr << strout.str(); }
    else                    { std::cout << strout.str(); }
