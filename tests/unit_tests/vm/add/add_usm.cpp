...
{ { UINT64_C(0x3FD12735D3224E60), UINT64_C(0xC005A26D910B44DC) }, { UINT64_C(0x4020A5D666294BAC), UINT64_C(0x402216D5173C2DAA) }, { UINT64_C(0x40212F1014C25E1F), UINT64_C(0x40195C7365F2B8E6) } }, //  3: vzAdd ( 0.268018203912310682      + i * -2.70431054416313366     , 8.32390136007401082       + i * 9.04459450349425609       ) = ( 8.59191956398632151       + i * 6.34028395933112243       );
  ...
  // Get device name
  std::string dev_name =
      main_queue.get_device().get_info<info::device::name>();
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    for (int i = 0; i < VLEN; ++i) {
      ...
      errs += check_result<A, R>("", "add", ",simple", i, argtype, acc,
...
bool vAddComplexSinglePrecisionAccuracyLiteTest_usm(
    device* dev) {
...
bool vAddComplexDoublePrecisionAccuracyLiteTest_usm(
    device* dev) {
  return vAddAccuracyLiteTest<std::complex<double>, std::complex<double>>(dev);
}

class AddUsmTests
        : public ::testing::TestWithParam<cl::sycl::device*> {};

TEST_P(AddUsmTests, RealSinglePrecision) {
    EXPECT_TRUEORSKIP(::vAddSinglePrecisionAccuracyLiteTest_usm(GetParam()));
}

TEST_P(AddUsmTests, RealDoublePrecision) {
    EXPECT_TRUEORSKIP(::vAddDoublePrecisionAccuracyLiteTest_usm(GetParam()));
}

TEST_P(AddUsmTests, ComplexSinglePrecision) {
    EXPECT_TRUEORSKIP(::vAddComplexSinglePrecisionAccuracyLiteTest_usm(GetParam()));
}

TEST_P(AddUsmTests, ComplexDoublePrecision) {
    EXPECT_TRUEORSKIP(::vAddComplexDoublePrecisionAccuracyLiteTest_usm(GetParam()));
}

INSTANTIATE_TEST_SUITE_P(AddUsmTestsuite, AddUsmTests,
