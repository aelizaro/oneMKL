...
#ifndef _ONEMKL_VL_DETAIL_MKLGPU_CT_HPP__
#define _ONEMKL_VL_DETAIL_MKLGPU_CT_HPP__
...
oneapi::mkl::vm::mode get_mode(backend_selector<backend::mklgpu> selector) {
    // TO DO: add pre- and post conditions if needed
    // get_mode_precondition(selector.get_queue());
    auto mode = oneapi::mkl::vm::mklgpu::get_mode(selector.get_queue());
...
oneapi::mkl::vm::status get_status(backend_selector<backend::mklgpu> selector) {
    // TO DO: add pre- and post conditions if needed
    // get_status_precondition(queue);
    auto status = oneapi::mkl::vm::mklgpu::get_status(selector.get_queue());
...
oneapi::mkl::vm::status clear_status(backend_selector<backend::mklgpu> selector) {
    // TO DO: add pre- and post conditions if needed
    // clear_status_precondition(queue);
    auto status = oneapi::mkl::vm::mklgpu::clear_status(selector.get_queue());
...
#endif //_ONEMKL_VL_DETAIL_MKLGPU_CT_HPP_
